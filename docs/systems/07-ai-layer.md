...
The complete set — exactly six. Adding a seventh? Follow the pattern: a pure prompt builder, a call
...
| `lib/plan-schema.ts`, `lib/retrospective-schema.ts`, `lib/narrative-critic.ts` | Each bundles its zod schema + `Tool` + parse/format helpers. There is **no central tool registry**. | |
