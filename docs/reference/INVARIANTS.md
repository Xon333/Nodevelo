...
1. **The ledger is append-only.** Past `score-log.json` entries are frozen with their provenance stamps (FTP-used, calibration, fuel, form state). Only today's entry re-derives. LEDGER-1: a rebuild can never un-plan a frozen entry. LEDGER-2: normal merges never rewrite history. Scoring-logic changes apply forward, never retroactively.
...
12. **Deterministic numbers, LLM phrasing.** The model never computes a training or nutrition figure; it copies from tables/snapshots the engines built ([ADR-0002](../adr/0002-deterministic-numbers-llm-phrases.md)).
