...
| ID | Item |
|----|------|
| TODAY-2 | Power-zone bar: removed % text labels, hover tooltip shows `Z· %` |
| TODAY-3 | Trend Pulse volume: per-week hover (`Week of … : h`); removed misleading down-arrow → "this wk" |
| TODAY-5 | Energy unit kJ → kcal on ride card (audit other surfaces still open) |
...
| ID | S | Pri | Type | Item |
|----|---|-----|------|------|
| DI-1 | ☐ | P1 | bug | Analysis wrong when plan interval defs ≠ live-detected defs — reconcile before scoring |
| DI-2 | ☐ | P1 | bug | Interval power mis-read: Intervals.icu 540W vs app 445W (duration correct) |
...
| ID | S | Pri | Type | Item |
|----|---|-----|------|------|
| PW-7 | ☐ | P1 | bug | **SIT duration mismatch**: Intervals.icu 5×30s vs app plan 5×1m — single source, sync (blocks PW-2) |
| PW-8 | ☐ | P1 | bug | Power goal ignores KB: app showed 150%, KB says SIT is maximal 130–200% FTP — reference KB intensity defs |
