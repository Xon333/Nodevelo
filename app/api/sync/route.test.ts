...
describe("POST /api/sync — today-ride analysis path", () => {
  ...
  it("persists the interval-adherence stamp alongside the today-patch's executionScore (SIT-bug fix)", async () => {
    ...
    vi.mocked(api.fetchIntervals).mockResolvedValue([
      { type: "WORK", durationSec: 720, avgWatts: 190, npWatts: 192, avgHr: 155, startIndex: 0, endIndex: 100, avgGradientPct: null, groupId: null, zone: null },
      { type: "WORK", durationSec: 720, avgWatts: 190, npWatts: 192, avgHr: 155, startIndex: 200, endIndex: 300, avgGradientPct: null, groupId: null, zone: null },
      { type: "WORK", durationSec: 720, avgWatts: 190, npWatts: 192, avgHr: 155, startIndex: 400, endIndex: 500, avgGradientPct: null, groupId: null, zone: null },
    ]);
    await postSync();
  ...
  it("does NOT stamp `intervals` on a durability-templated today even when intervalComparison is non-null (Finding 3 guard)", async () => {
    ...
    vi.mocked(api.fetchIntervals).mockResolvedValue([
      { type: "WORK", durationSec: 240, avgWatts: 230, npWatts: 230, avgHr: 165, startIndex: 600, endIndex: 700, avgGradientPct: null, groupId: null, zone: null },
    ]);
    await postSync();
    expect(scoreEntries.find((e) => e.date === TODAY)?.intervals).toBeUndefined();
