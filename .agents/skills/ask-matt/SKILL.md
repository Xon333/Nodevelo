...
3. **Branch: is this a multi-session build?**
   ...
   Either way, **`/implement`** builds each issue by driving **`/tdd`** internally (one red-green slice at a time), then closes out by running **`/code-review`** before committing. Reach for **`/tdd`** on its own when you just want to build a concrete behaviour test-first without a full spec. Reach for **`/code-review`** to compare Standards + Spec, hunt defects skeptically, request an independent review, or evaluate review feedback.

   When a plan contains independent tasks, use **`/agent-orchestration`** to assign disjoint ownership, run ready work concurrently, review each task, and integrate the verified result.
