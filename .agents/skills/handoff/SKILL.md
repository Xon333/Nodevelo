...
4. **Preserve the task safely.** Commit only coherent, verified files owned by the task. If the task is
   complete, integrate through `npm run finish:agent-task`; otherwise leave its worktree and branch in
   place for the next session. Do not publish a partial task merely to end the session.
