...
- **In-app proven-workout library (generation-time reuse + Intervals.icu export)** — NodeVelo builds a
  ...
  deferred until the manual path shows real usage. 3 of 10 planned tasks shipped: `lib/workout-library.ts`
  ...
  `CRITICAL` set), and `lib/workout-library-export.ts` + `lib/intervals-api.ts` additions (2026-08-16;
  single-flight + remote-lookup-before-create export, since the live API has no native upsert on this
  endpoint — confirmed via forum research, not assumed) + tests, tracked at
  [FILE_INDEX.md](docs/FILE_INDEX.md). **Not wired in** — no `app/api/generate` integration, no
  `app/api/write` use-count accounting, no API route, no management UI. Remaining 7 tasks + full design:
  `docs/superpowers/plans/2026-08-02-proven-workout-library-generation.md`.
