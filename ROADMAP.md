...
| **Adaptive self-directed coach — Phase 2** | Phase 1 landed on `claude/adaptive-coach-p1-aerobic-eligibility` (2026-08-06): a shared variability threshold now gates both `isSteadyEnduranceRide` (whole-ride comparability) and `qualifyingPwHr` (Pw:HR baseline membership); off-plan and planned-but-surgy rides no longer take circular VI/Pw:HR penalties. Historical entries already frozen before this landed (e.g. 2026-08-05/06's `2/10`) are unaffected until a ledger rebuild is explicitly triggered — not part of this phase. Phase 2 (intent overlay) is next; Phases 3–4 (weekly envelope, historical repair) not started | [plan](docs/superpowers/plans/2026-08-06-adaptive-coach-p1-aerobic-eligibility.md) |
