...
// PRESCRIBED session type (lib/plan-schema.ts's WORKOUT_TYPES enum, which gates what block generation
// may legally prescribe) — reusing its exact name for an off-plan inferred bucket misleads on the
// Trends hover title (components/trends/sections.tsx). Display-layer only, deliberately: the stored
// WorkoutType/inferredType value is untouched, so this can't expand what the LLM is allowed to
