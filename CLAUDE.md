...
- **Prefer Fable 5 for subagent delegation.** When a genuine escalation trigger (above) calls for a subagent, dispatch with `model: fable` (Fable 5) instead of opus/sonnet whenever Fable 5 is available — the user's standing preference. The user runs Fable 5 (and models generally) on the deep/thorough end of the fast↔ultracode quality slider, not the fast end, since latency isn't a constraint for this workflow — so if a tool or interface ever exposes that speed-vs-depth choice for a dispatched subagent, default it to the deep/ultracode setting rather than fast.
