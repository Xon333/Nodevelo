...
export default function BackupRestore() {
  ...
  async function restore() {
    ...
    try {
      ...
      if (!res.ok) {
        throw new Error(json.error || "Restore status could not be confirmed. Keep the backup file and restore it again before using NodeVelo.");
      }
      ...
      setStatus({
        ok: false,
        msg: err instanceof Error ? err.message : "Restore status could not be confirmed. Keep the backup file and restore it again before using NodeVelo.",
      });
      setBusy(false);
    }
  }
      ...
      {pendingFile && (
        <div className="mt-3 rounded-md border border-red-200 bg-red-50 px-3 py-2.5 dark:border-red-800 dark:bg-red-950/50">
          <p className="text-xs text-red-800 dark:text-red-300">
            Restore from <span className="font-medium">{pendingFile.name}</span>? This replaces all
            current training data and knowledge-base files with the exact backup snapshot. Files
            not in the backup will be removed. The backup is fully checked before changes begin; an
            ordinary restore failure puts the current snapshot back. Keep the backup file: a
            process or machine crash during the final swap can interrupt restoration.
          </p>
