...
export default function IfBandOffsets({ rows }: { rows: IfBandOffsetRow[] }) {
  ...
  const adjustedCount = rows.filter((r) => r.offset !== 0).length;
  return (
      ...
      {!hasZones ? (
        ...
        <details>
          <summary className="cursor-pointer select-none text-xs text-zinc-500 dark:text-zinc-400">
            Your IF bands vs population defaults
            {adjustedCount > 0 ? ` · ${adjustedCount} shifted to your zones` : " · using population defaults"}
          </summary>
